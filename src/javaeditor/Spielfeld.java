...
public class Spielfeld {
  ...
  public void initSpiel() {
    ...
    // Declare items
    Item feuerstab = new Item("Feuerstab", 30, 10);
    Item zauberumhang = new Item("Zauberumhang", 50, 10);
    Item zauberbuch = new Item("Zauberbuch", 20, 10);
    ...
    final String ANSI_YELLOW = "";
    final String ANSI_BLUE = "";
    final String ANSI_PURPLE = "";
    final String ANSI_CYAN = "";
    String fW = "";
    String fG = "";
    ...
    while (spieler.getEnergie() > 0) {
      ...
      System.out.println("- " + ANSI_CYAN + "sleep" + ANSI_RESET + "\n    Sleep to regenerate some of your energy");
      ...
      switch (befehl) {
          ...
          switch (antwort) {
            case "up":
              if (spieler.getYPos() > 0 && raum[spieler.getXPos()][spieler.getYPos() - 1].getSued() == true) {
                spieler.setLastSleept(spieler.getLastSleept() + 1);
        ...
        case "take":
          ...
          if (takeable == false) {
            ...
            if (raum[spieler.getXPos()][spieler.getYPos()].getmonsterBeaten() == true) {
              ...
              spieler.setZKraft(
                  spieler.getZKraft() + raum[spieler.getXPos()][spieler.getYPos()].getItem().getenergieIncrease());
              try {
                Thread.sleep(2000);
              } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
        ...
        case "fight":
          ...
          int damage = (int) (Math.round(spieler.getZKraft()) - monster.getDefence());
          if (damage <= 0) {
            ...
            System.out.println("Du hast dem Monster " + damage + " Schaden zugefuegt und das Monster hat dir "
                + monster.getZKraft() + " hinzugefügt");
        ...
        case "sleep":
          if (spieler.getLastSleept() >= 2) {
            System.out.println("Du hast erfolgreich geschlafen und 35 energie regeneriert.");
            spieler.setEnergie(spieler.getEnergie() + 35);
            spieler.setLastSleept(0);
            try {
              Thread.sleep(1000);
            } catch (InterruptedException e) {
              Thread.currentThread().interrupt();
            ...
            System.out.println(
                "Du kannst nicht schlafen, weil ist nicht lange genug her ist, dass du das etzte mal geschlafen hast.");
            try {
              Thread.sleep(1000);
            } catch (InterruptedException e) {
              Thread.currentThread().interrupt();
      ...
      if (raum[0][0].getmonsterBeaten() &&
          raum[0][1].getmonsterBeaten() &&
          raum[0][2].getmonsterBeaten() &&
          raum[1][2].getmonsterBeaten() &&
          raum[2][0].getmonsterBeaten() &&
          raum[2][2].getmonsterBeaten()) {
        System.out
            .println(ANSI_GREEN + "Herzlichen glueckwunsch, du hast Schloss wieder Bewohnbar gemacht" + ANSI_RESET);
        return;
      }
